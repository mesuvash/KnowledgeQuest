...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3 * 32 * 32, hidden_dim=100, num_classes=10,
        ...
        self.params["W2"] = np.random.randn(hidden_dim, num_classes) * 0.01
    ...
    def loss(self, X, y=None):
        ...
        #######################################################################
        hidden_activation, cache = affine_relu_forward(X, W1, b1)
        ...
        grads["W2"] = hidden_activation.T.dot(dout) + self.reg * W2
        grads["b2"] = dout.sum(axis=0)

        dh1 = dout.copy()
        # dh1[np.arange(X.shape[0]), y] -= 1
        dh1 = dh1.dot(W2.T)
        dx, dW1, grads["b1"] = affine_relu_backward(dh1, cache)
        dW1 += self.reg * W1
        grads["W1"] = dW1
        #######################################################################
        #                             END OF YOUR CODE                             #
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i in range(1, self.num_layers):
            ...
            self.params["W%d" % i] = np.random.randn(
                indim, outdim) * weight_scale
            self.params["b%d" % i] = np.zeros((1, outdim))
            if self.use_batchnorm:
                self.params["gamma%d" % i] = np.ones((1, outdim))
                self.params["beta%d" % i] = np.zeros((1, outdim))
            indim = outdim
        i += 1
        self.params["W%d" % i] = np.random.randn(
            indim, num_classes) * weight_scale
        self.params["b%d" % i] = np.zeros((1, num_classes))
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1, self.num_layers):
            ...
            if self.use_dropout:
                ...
                gamma, beta = self.params["gamma%d" %
                                          i], self.params["beta%d" % i]
                out, cache = affine_batchnorm_relu_forward(
                    inp, W, b, gamma, beta, self.bn_params[i - 1])
            else:
        ...
        # of 0.5 to simplify the expression for the gradient.                      #
        #######################################################################
        # print caches[-1][0][0].T.shape, dout.shape
        W = self.params["W%d" % self.num_layers]
        grads["W%d" % self.num_layers] = caches[-1][0][0].T.dot(
            dout) + self.reg * W
        ...
        for i in range(self.num_layers - 1, 0, -1):
            ...
            if self.use_dropout:
                ...
                dx, dW, db, dgamma, dbeta = affine_batchnorm_relu_backward(delta, cache)
            else:
